...
class DebugLogic:
    ...
    def showSlice(self, nifti_img, slice, cmap=None):
        plt.imshow(nifti_img[:, :, slice], cmap=cmap)
        plt.show()

    def showSlices(self, nifti_imgs, slice):
        plt.figure(figsize=(12, 8))
        i = 0
        for nifti_img, cmap in nifti_imgs:
            i += 1
            plt.subplot(1, len(nifti_imgs), i)
            plt.imshow(nifti_img[:, :, slice], cmap=cmap)
            plt.title("i " + str(i))
        plt.show()
    ...
    def evaluate(self, img, seeds, ref_mask=None):
        ...
        for i, seed in enumerate(seeds):
            ...
            plt.title(f"Mask seed:({seed[0]},{seed[1]})")

            print(f"res{i}  , seed:{seed},   value:{img[seed[0], seed[1]]} ")
            masks.append(mask)

        if ref_mask is not None:
            plt.subplot(2, 3, 6)
            plt.imshow(ref_mask, cmap='binary')
            plt.title(f"Ref mask")
    ...
    def score(self, masks, ref_mask, seeds):
        ...
        for i in range(len(masks)):
            ...
            plt.subplot(2, 3, i + 1)
            plt.imshow(diff_img)
            plt.title(f"Mask seed:({seeds[i][0]},{seeds[i][1]}) DSS: {DSS:.2f}")

        plt.subplot(2, 3, len(masks) + 1)
        plt.imshow(ref_mask, cmap='binary')
        plt.title(f"Ref mask")
    ...
    def findSeedPoints(self, region, image, groundTruth=None):
        ...
        if groundTruth is not None:
            ...
            plt.imshow(binary_seg, cmap="binary")
        else:
            plt.figure(figsize=(12, 8))
        ...
        '''target = np.array([173, 174])
        matches = np.all(seeds == target, axis=1)
        if np.any(matches):
            print(np.where(matches))
            index = np.where(matches)[0][0]
            print("ITT ", seeds[index], len(seeds))
            seeds=np.delete(seeds, index, axis=0)
            print("REMOVED FROM ",seeds[index], len(seeds))
            print(f"{tuple(target)} found at index {index} in seeds.")
        '''
    ...
    '''
        marad a SplitnMerge + gmm el választunk seedpontokat
    '''
    def executeCustomAlg1(self, img):
        ...
        gmm =GaussianMixture(n_components=2, random_state=0) # sima + elváltozott területre osztályozunk valség modellel
        ...
        #todo régiónövelés  seed_indices -el
        return
    ...
    '''
    def gradientSeedSelection(self,img, seeds):
    ...
    def execAlg1Extra(self):
        ''' Itt vlmi extra tudást kéne bevinni nem független 8x8 blokkokat nézni
            Akár split merge utáni részre nézni csak, vagy ezeket a függetleneket súlyozni

        Overlappaing vagy sliding kisebb blokkok 8x8 helyett, akár többségi szavazással választani
            átlag helyett
             -maximumot vagy szórás nézése


        Pontozni Ground truth nélkül
            -régió homogenitás szórás entrópiából?
            -régió területe alapján (büntetni kicsi vagy nagy maszkokat)
            -Kompaktság , alak pl mennyire kompakt/kör alakú persze, nem igazán kör alakúak ezek sőt...
            - Többségi seed szavazás
                - több seed hasonló maszkok -> jó lehet
                - Ground truth helyett egymással milyen átfedésben vannak (Dice score)

            magas pontszám ha maszkok "egyetértenek"

        - Mondjuk súlyozva összeadjuk őket
            - kézzel, vagy tanítani vlmi félig felügyelt módszerrel


         '''
        return
    def execAlg2WGaussblur(self):
        '''
            Gaussblur/simítás a divergencia előtt mert elég zajos a hisztogram
            akár a div-nek a gradiense
            akár entropia-t használni filternek?

            '''
        return


    def execCombinedAlg(self):
        '''Kombinálni valahogy az első 2 -t
            pl.
            1. Skullstrip
            2. Mindkettő seedselekciót megcsinálni (a javítottal)
                - A: első alg seeds
                - B: második alg seeds
            3. Kombinálni a seedeket C pl.: A-ból azok akik B-ben vannak
            4. Extra feltételek seedekhez pl ez a gradientSeedSelection(),
                vagy az adott seed környezetében a lokális entrópia, legyen alacsony
            5. Régiónövelés
                - összesből akár külön
                - vagy egybetartozókból úgy mint 2-es algnál

                + más régiónövelés feltétel?
            6. Ha több mask van akkor pontozni ground truth nélkül
    ...
    def placeholder(self):
        ''' Még a konzin beszéltekből még akár párat időfgv-ben'''
        return
...
class QuadTree():
    ...
    def __init__(self, image, depth=3):
        ...
        plt.subplot(2, 2, 2)
        plt.imshow(self.correctedImage, cmap="gray")
    ...
    def split(self, start, length, depth, position='Root'):
        ...
        if depth <= 0:  # 3 mély lehet emrgelni. lentről felfele
            ...
            # print(self.getStat(start,length,position,depth))
    ...
    def isHomogen(self, start, length, position='', depth=100):
        ...
        hyperMaskNorm = norm_hist[
            ((bin_edges >= self.hyperintenseRange[0])[:-1] & (bin_edges <= self.hyperintenseRange[1])[:-1])]
        hypoMaskNorm = norm_hist[int(self.hypointenseRange[0] * 256): int(self.hypointenseRange[1] * 256)]
        '''if(depth==1 and np.mean(hyperMask)>0):
            plt.figure(figsize=(12,8))
            plt.subplot(1,3,1)
            plt.plot(hist)
            plt.plot(hyperMask)
            plt.title(str(len(hist))+"--"+str(len(hyperMask)))
            plt.subplot(1,3,2)
            plt.plot(P)
            #plt.imshow(self.correctedImage[start_row:start_row+row_length,start_col:start_col+col_length])
            plt.plot(hypoMask)
            plt.subplot(1,3,3)
            plt.imshow(self.image[start_row:start_row+row_length,start_col:start_col+col_length],cmap='gray')
